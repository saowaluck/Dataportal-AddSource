  ...
  it('should simulates click submit form when edit resource type Knowledge Post in fields', (done) => {
    ...
    moxios.wait(() => {
      ...
      request.respondWith({
        ...
        '<div class="twelve wide column"><div class="ui segment"><form class="ui form">' +
        '<div class="field"><label for="url">Position' +
        '<input type="text" name="position" placeholder="position" value="intern" required=""/></label></div>' +
        '<div class="field"><label for="slack">Slack<input type="text" name="slack" placeholder="Slack" value="kmk" required=""/>' +
        '</label></div><hr/><button class="ui primary button" type="submit">Update profile</button></form></div></div></div></div>')
